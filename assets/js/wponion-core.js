...
$wponion.tost = swal.mixin({
	...
	onOpen: function onOpen() {
		var $height = parseInt(jQuery("#wpadminbar").height()) + 3;
...
$wponion.validate_single_function = function ($string) {
	if ((typeof $string === 'undefined' ? 'undefined' : _typeof($string)) === 'object') {
		var $args = $string['wponion_js_args'] === false ? false : $string['wponion_js_args'];
		var $contents = $string['wponion_js_contents'] === false ? false : $string['wponion_js_contents'];
 ...
 * @returns {boolean}
 */
$wponion.valid_json = function (obj) {
	try {
		JSON.parse(obj);
		return true;
	} catch (e) {
		...
		var tippy_helper = {
			create_instance: function create_instance($elem, $arguments) {
				if ($elem.attr('data-tippy-instance-id') === undefined) {
					var $_instance_id = 'Tippy' + $wponion.randomMD5();
...
var $wpo_modal_app = Backbone.View.extend({
	/**
  ...
  */
	init_templates: function init_templates() {
		var $modal = $wponion.settings('modal');
		...
		if (this.html) {
			_.each($this.html, function (value, key) {
				if (typeof value['sections'] !== "undefined") {
					var $content = $this.templates.page_content({
						id: key,
						title: value['title'],
						html: value['html']
					});
					...
					_.each(value['sections'], function (val, k) {
						var $_content = $this.templates.section_content({
							id: key + "_" + k,
							title: val['title'],
							html: val['html']
						});
						var $_menu = $this.templates.router_menu_item({ url: k, name: val['title'] });
					...
					var _$content = $this.templates.page_content({
						id: key,
						title: value['title'],
						html: value['html']
					});
					...
					_$content = $(_$content);
					_$content.find('.media-sidebar').hide();

					if (typeof value['sidebar'] !== "undefined") {
						if (value['sidebar'] !== false) {
							_$content.find('.media-sidebar').append(value['sidebar']).show();
						}
					}

					_$content.find('.media-frame-router').addClass('hidden');
					$this.$('.wponion-modal-content-container').append(_$content);
				}
			});
		}

		$this.$('.media-menu a:first-child').trigger('click');
		$this.$('.wponion-modal-content-container > .wponion-modal-content:not(.hidden) .media-frame-router a:first-child').trigger('click');
  ...
  */
	handle_left_menu_click: function handle_left_menu_click(e) {
		...
		var $show_target = $(this.$el).find('.wponion-modal-content-container > div#' + $target.attr('href'));
	...
	handle_tab_click: function handle_tab_click(e) {
		...
		var $page = $(this.$el).find('.media-frame-menu a.active').attr('href');
		var $base = $(this.$el).find('.wponion-modal-content-container > #' + this.active_page);
...
var WPO_Modal = function () {
	function WPO_Modal($options) {
		_classCallCheck(this, WPO_Modal);
