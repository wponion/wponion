( ( window, document, $, wpo, wpt, wp ) => {
	 ...
	 * @constructor
	 */
	wpt.fn.WPO_Modern_submenu = function () {
		let $this = this;
		$this.elem.find( '.wponion-submenus.subsubsub a' ).on( 'click', function ( e ) {
			...
			if ( $href[ 'section-id' ] !== undefined && $href[ 'parent-id' ] !== undefined ) {
				...
				$( '.wponion-framework.wponion-module-settings .page-loader' ).show();
	 ...
	 * @constructor
	 */
	wpt.fn.WPO_Modern_main_menu = function () {
		let $this = this;
	...
	wphooks.addAction( 'wponion_before_init', ( () => {
		...
		if ( $elem.hasClass( 'wponion-submenu-single-page' ) ) {
			wponion_theme( '.wponion-framework' ).WPO_Modern_submenu();
		}

		if ( $elem.hasClass( 'wponion-single-page' ) ) {
			wponion_theme( '.wponion-framework' ).WPO_Modern_main_menu().WPO_Modern_submenu();
		}
	} ) );
