...
/**
 *
 * Initial version created 17-06-2018 / 07:04 AM
 *
...
if ( ! class_exists( '\WPOnion\Modules\Customize\Postmessage' ) ) {
	/**
	 * Class postmessage
	 *
	 * @package WPOnion\Modules\Customize
	 * @author Varun Sridharan <varunsridharan23@gmail.com>
	 * @since 1.0
	 */
	class postmessage extends \WPOnion\Bridge\Module {
		...
		protected function handle_field_script( $field = array() ) {
		...
		protected function script( $args ) {
			...
			if ( $add_css ) {
				...
				// The style ID.
				$style_id = 'wponion-postmessage-' . str_replace( array( '[', ']' ), '', $args['settings'] );
		...
		protected function script_var_typography( $args, $field ) {
			...
			foreach ( $css_build_array as $property => $var ) {
				...
				if ( ! isset( $field['default'] ) || ( ( 'font-family' === $property && ! isset( $field['default']['font-family'] ) ) || ( 'font-size' === $property && ! isset( $field['default']['font-size'] ) ) || ( 'line-height' === $property && ! isset( $field['default']['line-height'] ) ) || ( 'letter-spacing' === $property && ! isset( $field['default']['letter-spacing'] ) ) || ( 'word-spacing' === $property && ! isset( $field['default']['word-spacing'] ) ) || ( 'text-align' === $property && ! isset( $field['default']['text-align'] ) ) || ( 'text-transform' === $property && ! isset( $field['default']['text-transform'] ) ) || ( 'text-decoration' === $property && ! isset( $field['default']['text-decoration'] ) ) || ( 'color' === $property && ! isset( $field['default']['color'] ) ) || ( 'font-weight' === $property && ! isset( $field['default']['variant'] ) && ! isset( $field['default']['font-weight'] ) ) || ( 'font-style' === $property && ! isset( $field['default']['variant'] ) && ! isset( $field['default']['font-style'] ) ) ) ) {
					continue;
				}
